...
import type { Mood } from './sprites.js'; // Pet mood type
 ...
 */
export function getMessage(mood: Mood): string {
...
export function getContextMessage(
  ...
  const contextObj = contextMessages[context];
  ...
  if (typeof contextObj === 'object' && !Array.isArray(contextObj) && subContext) {
    ...
    pool = (contextObj as Record<string, string[]>)[subContext] || [];
 ...
 */
export function getRepoFunFact(data: {
  days?: number;           // Days since first commit
  commits?: number;        // Total commit count
  day?: string;            // Most active day (not always available)
  avgLength?: number;      // Average commit message length
  topExt?: string;         // Most common file extension
  count?: number;          // Count of most common extension
}): string {
  ...
  if (data.days !== undefined) {
    availableFacts.push(repoFunFacts[0].replace('{days}', String(data.days)));
  }

  if (data.commits !== undefined) {
    availableFacts.push(repoFunFacts[1].replace('{commits}', String(data.commits)));
  }

  if (data.day !== undefined) {
    availableFacts.push(repoFunFacts[2].replace('{day}', data.day));
  }

  if (data.avgLength !== undefined) {
    availableFacts.push(repoFunFacts[3].replace('{avgLength}', String(data.avgLength)));
  }

  if (data.topExt !== undefined && data.count !== undefined) {
    availableFacts.push(
      repoFunFacts[4].replace('{topExt}', data.topExt).replace('{count}', String(data.count))
    );
  }
