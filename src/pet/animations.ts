...
export interface AnimationController {
  ...
  position: number;        // Horizontal position offset
  frame: number;           // Current frame within the animation
  stateStartTime: number;  // When this state started
  stateDuration: number;   // How long this state should last
  lastMood: string;        // Track mood changes
}
...
const HAPPY_BEHAVIORS: AnimationState[] = [
  'walking_right',
  'walking_left',
  'walking_right',
  'walking_left',
...
const SAD_BEHAVIORS: AnimationState[] = [
  'idle',
  'whimpering',
  'whimpering',
  'head_down',
 ...
 */
function pickRandomState(options: AnimationState[], mood: string): AnimationState {
 ...
 */
function pickNextState(mood: string): AnimationState {
...
export function getAnimatedSprite(
  ...
  switch (controller.state) {
    ...
    case 'rolling':
      ...
      if (rollPhase < 2) return puppyRoll1;
      if (rollPhase < 4) return puppyRoll2;
      return puppyRoll3;

    case 'tail_chase':
      const chasePhase = frame % 6;
      if (chasePhase < 2) return puppyTailChase1;
      if (chasePhase < 4) return puppyTailChase2;
 ...
 */
export function applyPosition(sprite: string[], position: number): string[] {
  const offset = Math.round(position);

  if (offset === 0) return sprite;
