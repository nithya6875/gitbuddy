...
import type { Level } from '../pet/sprites.js'; // Level type: 1 | 2 | 3 | 4 | 5
 ...
 */
export const levelThresholds: Record<Level, number> = {
 ...
 */
export const xpToNextLevel: Record<Level, number> = {
...
export function getXPProgress(xp: number): { current: number; needed: number } {
  ...
  const nextThreshold = levelThresholds[(Math.min(level + 1, 5)) as Level];
