...
exports.getSpriteHeight = getSpriteHeight;
exports.addBreathing = addBreathing;
exports.getLevelName = getLevelName;
const chalk_1 = __importDefault(require("chalk")); // Terminal string styling
const colors_js_1 = require("../ui/colors.js"); // Color theme definitions
// =============================================================================
// SPRITE COLORIZATION
// =============================================================================
/**
 ...
 */
function colorize(sprite) {
    return sprite
        // Box drawing characters - dog body outline
        .replace(/[╭╮╰╯│─┌┐└┘├┤┬┴┼]/g, (m) => colors_js_1.colors.dogBody(m))
        // Eye characters - white for visibility
        .replace(/[◕◔●◠◡×·ᵔ^]/g, (m) => chalk_1.default.white(m))
        // Nose/mouth characters - dark brown
        .replace(/[▽ᴥ▾◡]/g, (m) => colors_js_1.colors.dogNose(m))
        // Hearts - red for love
        .replace(/[♥❤]/g, (m) => colors_js_1.colors.heart(m))
        // Stars and sparkles - yellow
        .replace(/[★✦✧⭐]/g, (m) => colors_js_1.colors.sparkle(m))
        // Tears - cyan (water color)
        .replace(/[💧]/g, (m) => colors_js_1.colors.tear(m))
        // Sleep indicators - dimmed
        .replace(/[Zz]/g, (m) => colors_js_1.colors.sleeping(m))
        // Fur texture blocks - light brown
        .replace(/[░▒▓█]/g, (m) => colors_js_1.colors.dogBodyLight(m))
        // Tail wagging indicators
        .replace(/[∼∽~]/g, (m) => colors_js_1.colors.dogBody(m))
        // Music notes - green (happy sound)
        .replace(/[♪♫]/g, (m) => colors_js_1.colors.happy(m));
}
// =============================================================================
 ...
 */
function getSprite(mood, level, frame, isBlinking = false) {
 ...
 */
function renderSprite(sprite, width = 40) {
    return sprite
        .map((line) => {
 ...
 */
function getSpriteHeight(level) {
