...
const log_update_1 = __importDefault(require("log-update")); // In-place terminal updates
...
class Renderer {
    ...
    messageChangeTime = Date.now();
    /** How often to rotate messages in milliseconds */
    messageInterval = 8000; // ms
    ...
    updateAnimations() {
        ...
        if (this.renderCallback && now - this.messageChangeTime > this.messageInterval) {
            ...
            if (!state.customMessage) {
                this.currentMessage = (0, dialogue_js_1.getMessage)(state.mood);
    ...
    render() {
        ...
        if (this.useDynamicAnimations && state.mood !== 'sleeping' && state.mood !== 'eating') {
            ...
            this.animationController = (0, animations_js_1.updateAnimation)(this.animationController, state.mood);
            // Level 1 has custom animated sprites
            if (state.level === 1) {
                sprite = (0, animations_js_1.getAnimatedSprite)(this.animationController, state.level, state.mood);
            }
            else {
                // Higher levels use standard sprites but still move
                sprite = (0, sprites_js_1.getSprite)(state.mood, state.level, this.frame, this.isBlinking);
            }
            // Apply position offset for movement (pet walks across terminal)
            sprite = (0, animations_js_1.applyPosition)(sprite, this.animationController.position);
        }
        else {
            ...
            if (state.mood !== 'sleeping' && state.mood !== 'sick') {
                sprite = (0, sprites_js_1.addBreathing)(sprite, this.frame);
            }
        }
        // ---- Build Layout Data ----
        const layoutData = {
        ...
        const screen = (0, layout_js_1.buildLayout)(layoutData);
        (0, log_update_1.default)(screen);
    }
    // -------------------------------------------------------------------------
    ...
    renderCustom(content) {
        ...
        (0, log_update_1.default)(content);
    }
    // -------------------------------------------------------------------------
    // Message Control
    // -------------------------------------------------------------------------
    ...
    setMessage(message, duration = 3000) {
        ...
        setTimeout(() => {
            ...
            this.currentMessage = (0, dialogue_js_1.getMessage)(originalGetState().mood);
        }, duration);
    }
    // -------------------------------------------------------------------------
