...
exports.getHoursSinceLastVisit = getHoursSinceLastVisit;
exports.calculateHPDecay = calculateHPDecay;
exports.addXP = addXP;
exports.addAchievement = addAchievement;
exports.resetState = resetState;
exports.checkAndResetDailyCounters = checkAndResetDailyCounters;
exports.incrementDailyCounter = incrementDailyCounter;
exports.recordSessionAction = recordSessionAction;
const fs = __importStar(require("fs")); // Node.js file system module for reading/writing files
const path = __importStar(require("path")); // Node.js path module for handling file paths
const os = __importStar(require("os")); // Node.js OS module for getting home directory
// =============================================================================
// FILE PATH CONSTANTS
// =============================================================================
/**
 ...
 * }
 */
function getHoursSinceLastVisit(state) {
 ...
 * }
 */
function calculateHPDecay(state) {
...
function addXP(state, amount) {
    ...
    let newLevel = 1;
    if (newXP >= 1000)
        newLevel = 5; // Legendary Doge
    else if (newXP >= 600)
        newLevel = 4; // Cool Dog
    else if (newXP >= 300)
        newLevel = 3; // Adult Dog
    else if (newXP >= 100)
        newLevel = 2; // Young Dog
 ...
 */
function checkAndResetDailyCounters(state) {
    const today = new Date().toISOString().split('T')[0];
 ...
 */
function incrementDailyCounter(state, counter) {
    if (counter === 'date')
        return state;
    ...
    return updateState({
        dailyCounters: {
            ...dailyCounters,
            [counter]: (dailyCounters[counter] || 0) + 1,
        },
    });
}
/**
 ...
 */
function recordSessionAction(state, action) {
    // Handle missing actionsThisSession (old save file)
    const actionsThisSession = state.actionsThisSession || [];
    if (actionsThisSession.includes(action))
        return state;
    return updateState({
