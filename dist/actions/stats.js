...
exports.buildStatsScreen = buildStatsScreen;
...
function buildStatsScreen(data) {
    ...
    lines.push(colors_js_1.colors.frame('│') + chalk_1.default.bold.yellow(`  📊 ${state.name}'s Statistics`) + ' '.repeat(width - 23 - state.name.length) + colors_js_1.colors.frame('│'));
    lines.push(colors_js_1.colors.frame('│') + ' '.repeat(width - 2) + colors_js_1.colors.frame('│'));
    // -------------------------------------------------------------------------
    // Pet Info Section - Core pet statistics
    // -------------------------------------------------------------------------
    lines.push(colors_js_1.colors.frame('│') + chalk_1.default.bold('  🐕 Pet Info') + ' '.repeat(width - 16) + colors_js_1.colors.frame('│'));
    // Level display with title and stars
    // getLevelTitle returns names like "Puppy", "Junior", "Good Boy", etc.
    lines.push(colors_js_1.colors.frame('│') + `     Level: ${state.level} (${(0, xp_js_1.getLevelTitle)(state.level)}) ${(0, colors_js_1.getLevelStars)(state.level)}` + ' '.repeat(width - 38 - (0, xp_js_1.getLevelTitle)(state.level).length) + colors_js_1.colors.frame('│'));
    // XP display
    lines.push(colors_js_1.colors.frame('│') + `     XP: ${state.xp}` + ' '.repeat(width - 12 - String(state.xp).length) + colors_js_1.colors.frame('│'));
    // HP display with max value
    lines.push(colors_js_1.colors.frame('│') + `     HP: ${state.hp}/100` + ' '.repeat(width - 16 - String(state.hp).length) + colors_js_1.colors.frame('│'));
    // -------------------------------------------------------------------------
    ...
    lines.push(colors_js_1.colors.frame('│') + `     Days Together: ${daysTogether}` + ' '.repeat(width - 23 - String(daysTogether).length) + colors_js_1.colors.frame('│'));
    // Section separator
    lines.push(colors_js_1.colors.frame('│') + ' '.repeat(width - 2) + colors_js_1.colors.frame('│'));
    // -------------------------------------------------------------------------
    ...
    lines.push(colors_js_1.colors.frame('│') + `     Total Feeds: ${state.totalFeeds}` + ' '.repeat(width - 21 - String(state.totalFeeds).length) + colors_js_1.colors.frame('│'));
    // Total play actions performed
    lines.push(colors_js_1.colors.frame('│') + `     Total Plays: ${state.totalPlays}` + ' '.repeat(width - 21 - String(state.totalPlays).length) + colors_js_1.colors.frame('│'));
    // Longest commit streak ever achieved (personal best)
    lines.push(colors_js_1.colors.frame('│') + `     Longest Streak: ${state.longestStreak} days` + ' '.repeat(width - 28 - String(state.longestStreak).length) + colors_js_1.colors.frame('│'));
    // Total number of repository scans
    lines.push(colors_js_1.colors.frame('│') + `     Total Scans: ${state.totalScans}` + ' '.repeat(width - 21 - String(state.totalScans).length) + colors_js_1.colors.frame('│'));
    // Section separator
    lines.push(colors_js_1.colors.frame('│') + ' '.repeat(width - 2) + colors_js_1.colors.frame('│'));
    // -------------------------------------------------------------------------
    // Repository Section - Git repository statistics
    // -------------------------------------------------------------------------
    lines.push(colors_js_1.colors.frame('│') + chalk_1.default.bold('  💻 Repository') + ' '.repeat(width - 18) + colors_js_1.colors.frame('│'));
    // Overall health score (calculated from various repo metrics)
    lines.push(colors_js_1.colors.frame('│') + `     Health Score: ${health.totalScore}/100` + ' '.repeat(width - 26 - String(health.totalScore).length) + colors_js_1.colors.frame('│'));
    // Total commit count in the repository
    lines.push(colors_js_1.colors.frame('│') + `     Total Commits: ${health.commitCount}` + ' '.repeat(width - 24 - String(health.commitCount).length) + colors_js_1.colors.frame('│'));
    // Current commit streak in days
    lines.push(colors_js_1.colors.frame('│') + `     Current Streak: ${health.streak} days` + ' '.repeat(width - 28 - String(health.streak).length) + colors_js_1.colors.frame('│'));
    // -------------------------------------------------------------------------
    // Last Commit Time - Human-readable format
    // -------------------------------------------------------------------------
    if (health.lastCommitDate) {
        const lastCommit = formatTimeAgo(health.lastCommitDate);
        lines.push(colors_js_1.colors.frame('│') + `     Last Commit: ${lastCommit}` + ' '.repeat(width - 21 - lastCommit.length) + colors_js_1.colors.frame('│'));
    }
    // Section separator
    lines.push(colors_js_1.colors.frame('│') + ' '.repeat(width - 2) + colors_js_1.colors.frame('│'));
    // -------------------------------------------------------------------------
    ...
    if (state.achievements.length > 0) {
        lines.push(colors_js_1.colors.frame('│') + chalk_1.default.bold('  🏆 Achievements') + ' '.repeat(width - 20) + colors_js_1.colors.frame('│'));
        // Show up to 3 achievements with star icons
        for (const achievement of state.achievements.slice(0, 3)) {
            lines.push(colors_js_1.colors.frame('│') + `     ⭐ ${achievement}` + ' '.repeat(Math.max(0, width - 8 - achievement.length)) + colors_js_1.colors.frame('│'));
        }
        // If more than 3 achievements, show count of additional
        if (state.achievements.length > 3) {
            lines.push(colors_js_1.colors.frame('│') + colors_js_1.colors.textDim(`     ... and ${state.achievements.length - 3} more`) + ' '.repeat(width - 22 - String(state.achievements.length - 3).length) + colors_js_1.colors.frame('│'));
        }
        // Section separator
        lines.push(colors_js_1.colors.frame('│') + ' '.repeat(width - 2) + colors_js_1.colors.frame('│'));
    }
