...
const child_process_1 = require("child_process"); // Node.js module for executing shell commands
const util_1 = require("util"); // Utility to convert callbacks to promises
const fs = __importStar(require("fs")); // File system for checking file existence
const path = __importStar(require("path")); // Path utilities for file extensions
...
async function hasTestFiles() {
    try {
        ...
        for (const file of files) {
            if (file.includes('.test.') || // Jest/Mocha style: app.test.ts
...
async function scanRepo() {
    ...
    const testStatus = tests.found ? 'great' : 'warning';
    checks.push({
        name: 'Tests',
        status: testStatus,
    ...
    const readmeStatus = readme ? 'great' : 'warning';
    checks.push({
        name: 'README',
        status: readmeStatus,
    ...
    const totalScore = Math.round(checks.reduce((sum, c) => sum + (c.score * c.weight) / totalWeight, 0));
